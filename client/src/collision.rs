...
pub fn resolve_player_collisions(
    ...
    for p in platforms {
        if p.rect.check_collision_recs(&player_rect) {
            if player.vel.x > 0.0 {
                player.pos.x = p.rect.x - (player.size.x / 2.0);
            } else if player.vel.x < 0.0 {
                player.pos.x = p.rect.x + p.rect.width + (player.size.x / 2.0);
            }
            player.vel.x = 0.0;
        }
    }
    ...
    player_rect.x = player.pos.x - player.size.x / 2.0;
    player_rect.y = player.pos.y - player.size.y / 2.0;

    for p in platforms {
        if p.rect.check_collision_recs(&player_rect) {
            if player.vel.y > 0.0 {
                player.pos.y = p.rect.y - (player.size.y / 2.0);
            } else if player.vel.y < 0.0 {
                player.pos.y = p.rect.y + p.rect.height + (player.size.y / 2.0);
            }
            player.vel.y = 0.0;
        }
    }
